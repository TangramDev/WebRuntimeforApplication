...
void RecordLaunchModeHistogram(LaunchMode mode) {
    ...
    if (g_pCosmosImpl == nullptr)
    {
        ...
        if (hModule) {
          typedef CommonUniverse::CCosmosImpl*(__stdcall * GetCosmosImpl)(ICosmos**);
    ...
    if (g_pCosmosImpl && g_pCosmosImpl->m_nAppType == APP_BROWSER_ECLIPSE)
	    return;
  // end Add by TangramTeam
...
// begin Add by TangramTeam
class ChromeBrowserFactory : public CommonUniverse::CCosmosBrowserFactory {
 public:
...
HWND ChromeBrowserFactory::CreateBrowser(HWND hParent, CString strXml) {
  ...
  if (profile_) {
    ...
    if (b == true) {
      if (g_pCosmosImpl && ::IsWindow(hParent))
        g_pCosmosImpl->m_hParent = hParent;
      ...
      g_pCosmosImpl->m_hParent = NULL;
      g_pCosmosImpl->OnSubBrowserWndCreated(hParent, hBrowser);
      browser->window()->Show();
    }
  }
  return hBrowser;
...
bool StartupBrowserCreatorImpl::Launch(Profile* profile,
  ...
  if (!MaybeLaunchApplication(profile)) {
    ...
    // begin Add by TangramTeam
    if (g_pCosmosImpl && g_pCosmosImpl->m_pBrowserFactory == nullptr) {
      ChromeBrowserFactory* pImpl = new ChromeBrowserFactory();
      g_pCosmosImpl->m_pBrowserFactory = pImpl;
      pImpl->profile_ = profile_;
    }
    if ((g_pCosmosImpl && g_pCosmosImpl->m_nAppType == APP_BROWSER) || process_startup == false)
      // end Add by TangramTeam
  ...
  // begin Add by TangramTeam
  if (g_pCosmosImpl && g_pCosmosImpl->m_pBrowserFactory)
  {
	  g_pCosmosImpl->BrowserAppStart();
  }
  // end Add by TangramTeam
#endif  // defined(OS_WIN)
