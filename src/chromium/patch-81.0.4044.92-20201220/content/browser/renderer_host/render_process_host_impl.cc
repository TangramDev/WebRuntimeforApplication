...
class RendererSandboxedProcessLauncherDelegate
 ...
 public:
  ...
  service_manager::SandboxType GetSandboxType() override {
	  // begin Add by TangramTeam
	  if (g_pCosmosImpl) {
		  g_pCosmosImpl->OnRenderProcessCreated(nullptr);
	  }
	  // end Add by TangramTeam
...
bool RenderProcessHostImpl::Init() {
  ...
  CString path = _T("");
  if (g_pCosmosImpl) {
      path = g_pCosmosImpl->GetProcessPath(version_info::GetVersionNumber().c_str(),
          _T("renderer"));
  }
...
void RenderProcessHostImpl::OnProcessLaunched() {
  ...
  if (IsReady()) {
	...
	// begin Add by TangramTeam
	if (g_pCosmosImpl) {
		g_pCosmosImpl->OnRenderProcessCreated(this);
	}
	// end Add by TangramTeam
  }
